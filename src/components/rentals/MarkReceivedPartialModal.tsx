...
export interface MarkReceivedPartialModalProps {
...
export default function MarkReceivedPartialModal({
  collection,
  tenancy,
  remainingBalance,
  isSaving,
  ...
  return (
    ...
    >
      <div
        role="dialog"
        aria-labelledby="mark-partial-title"
        className="bg-white rounded-xl max-w-md w-full p-6 shadow-xl max-h-[90vh] overflow-y-auto"
        onClick={(e) => e.stopPropagation()}
      >
        <h2 id="mark-partial-title" className="text-lg font-semibold text-slate-900 mb-2">
        ...
        <form onSubmit={handleSubmit} className="flex flex-col gap-4">
          ...
          <label className="block">
            ...
            />
            {errors.amount && (
              <span className="text-red-600 text-xs mt-1 block">{errors.amount}</span>
            )}
          </label>
